...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, features_list, ori_patch = False, proj_use = True, DPAM_layer = None, ffn = False):
        ...
        transformer_output = self.transformer(x, features_list, DPAM_layer = DPAM_layer, ffn = ffn)
        
        if DPAM_layer is None:
            [x, x_ori], patch_tokens = transformer_output
        else:
            x, patch_tokens = transformer_output
            # When DPAM_layer is not None, x might be a list [x, x_ori]
            # where x[0] is the modified path and x[1] is the original path
            if isinstance(x, list):
                if len(x) >= 2:
                    x, x_ori = x[0], x[1]  # x is modified path, x_ori is original path
                else:
                    # Fallback: if list has only one element, use it for both
                    x = x[0]
                    x_ori = x
            else:
                x_ori = x
        ...
        if True:
            ...
            '''
            # Ensure x_ori is a tensor, not a list
            # Recursively extract tensor from nested lists if needed
            while isinstance(x_ori, list):
                if len(x_ori) > 0:
                    x_ori = x_ori[-1]  # Take the last element (original path)
                else:
                    raise ValueError("x_ori is an empty list")
            
            # x_ori should now be a tensor with shape [L, N, D], need to permute to [N, L, D] before indexing
            if not isinstance(x_ori, torch.Tensor):
                raise TypeError(f"x_ori must be a tensor, but got {type(x_ori)}")
            x_ori = x_ori.permute(1, 0, 2)  # LND -> NLD, shape: [N, L, D]
            # Only take the class token (index 0) for global feature, shape: [N, D]
...
class VisualAdapter(nn.Module):
    ...
    def forward(self, image_features, patch_features, static_text_features, control=None):
        ...
        if control is not None:
            control_feature = control[-1]
            patch_feature = patch_feature + control_feature
