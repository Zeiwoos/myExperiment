...
import adapterlib
from adapterlib import ControlNet, PQAdapter, TextualAdapter, VisualAdapter, fusion_fun
...
def test(args):
    ...
    if controlnet is not None:
        if "controlnet" not in checkpoint_adapter:
            raise ValueError("--enable_controlnet is set, but checkpoint has no 'controlnet' weights")
        ...
        if args.visual_learner:
            ...
            if controlnet is not None:
                if args.control_hint_source == "gt":
                    hint = gt_mask.unsqueeze(1)
                else:
                    hint = query_image
                hint = prepare_control_hint(hint, args.control_hint_channels)
                control_features = controlnet(query_patch_feats, hint)

            global_vl_logit, local_vl_map = visual_learner(query_feats, query_patch_feats, static_text_features, control=control_features)
    ...
    if len(gt_px_all_flat) > 0 and len(pr_px_all_flat) > 0:
        ...
        for threshold in f1_thresholds:
            try:
                ...
                f1_results_dict[threshold] = None
                logger.warning(f"无法计算阈值 {threshold:.6f} 下的pixel-level F1: {e}")
    else:
        # 数据为空或无效
        overall_seg_auc = 0.0
        au_pro_005 = 0.0
    ...
    # 输出pixel-level F1结果（多个阈值）
    if len(f1_results_dict) > 0:
        logger.info(f"\nOverall Pixel-level F1 (多个阈值):")
        for threshold in sorted(f1_results_dict.keys()):
            f1_result = f1_results_dict[threshold]
            if f1_result is not None:
                logger.info(f"  阈值 {threshold:.6f}: F1={f1_result.f1:.4f}, Precision={f1_result.precision:.4f}, Recall={f1_result.recall:.4f}")
    
    logger.info("="*80)

    # ====================== A/B/C 类及困难项统计（基于前缀与 hardClass.txt） ======================
    logger.info("\n" + "="*80)
    logger.info("按 A/B/C 及其困难项统计 Seg AUC 与 AU-PRO...")

    # 读取困难类别列表（子文件夹名中包含这些字段即视为困难项）
    hard_classes_path = os.path.join(os.path.dirname(__file__), "hardClass.txt")
    hard_class_tokens = set()
    if os.path.exists(hard_classes_path):
        with open(hard_classes_path, "r", encoding="utf-8") as f:
            for line in f:
                token = line.strip()
                if token:
                    hard_class_tokens.add(token)
    else:
        logger.warning(f"未找到 hardClass.txt 文件: {hard_classes_path}，将不进行困难项划分")

    # 定义前缀到类别的映射
    prefix_A = ("diushi_", "songdong_")
    prefix_B = ("yiwu_", "louyou_", "posun_")

    # 结果中 cls_names 仍为 torch.Tensor，需要转为 numpy / list
    if torch.is_tensor(results_eval["cls_names"]):
        cls_names_np = results_eval["cls_names"].cpu().numpy()
    else:
        cls_names_np = np.array(results_eval["cls_names"])

    # 统一转为字符串数组
    cls_names_str = np.array([str(x) for x in cls_names_np])
    ...
    # 遍历所有样本，按规则划分到 A/B/C 及其困难项
    for idx in range(len(query_paths_array)):
        img_path = query_paths_array[idx]
        anomaly_flag = int(gt_anomalys_np[idx])
        cls_name = cls_names_str[idx]
        ...
        # A/B 仅来自 anomaly_query，C 来自 normal_query
        if anomaly_flag == 1 and "anomaly_query" in img_path_str:
            if basename.startswith(prefix_A):
                category = "A"
            elif basename.startswith(prefix_B):
                category = "B"
        elif anomaly_flag == 0 and "normal_query" in img_path_str:
            category = "C"

        if category is None:
            continue

        # 是否为困难项：子文件夹名称（cls_name）中包含 hardClass.txt 中的字段
        is_hard = False
        for token in hard_class_tokens:
            if token in cls_name:
                is_hard = True
                break

        subsets[category].append(idx)
        if is_hard:
            hard_key = f"{category}_hard"
            if hard_key in subsets:
                subsets[hard_key].append(idx)

    def compute_subset_metrics(indices, compute_f1=False):
        """给定样本索引集合，计算 Seg AUC、AU-PRO@0.05/@0.30 和可选的多个阈值下的 F1。
        
        Args:
            indices: 样本索引列表
            compute_f1: 是否计算 F1 指标（仅用于 A、B 及其困难项）
        
        Returns:
            (count, seg_auc, au_pro_005, au_pro_030, f1_results_dict)
            f1_results_dict 为字典 {threshold: F1Result}，如果 compute_f1=False 或计算失败则为 {}
        """
        if len(indices) == 0:
            return 0, 0.0, 0.0, 0.0, {}

        indices = np.array(indices)
        sub_gt = gt_masks_np[indices]
        sub_pr = pr_masks_np[indices]

        gt_flat = sub_gt.ravel()
        pr_flat = sub_pr.ravel()

        if len(gt_flat) == 0 or len(pr_flat) == 0:
            return len(indices), 0.0, 0.0, 0.0, {}
        ...
        # 数据有效性检查
        if (
            not np.any(np.isnan(gt_flat))
            and not np.any(np.isnan(pr_flat))
            and not np.any(np.isinf(gt_flat))
            and not np.any(np.isinf(pr_flat))
            and len(np.unique(gt_flat)) >= 2
        ):
            try:
                seg_auc_val = roc_auc_score(gt_flat, pr_flat)
                if not np.isnan(seg_auc_val):
                    seg_auc = float(seg_auc_val)
            except ValueError as e:
                logger.warning(f"无法计算子集 Seg AUC（样本数={len(indices)}）: {e}")

        # AU-PRO
        try:
            sub_au_pro_005 = float(au_pro(sub_pr, sub_gt, max_fpr=0.05))
            sub_au_pro_030 = float(au_pro(sub_pr, sub_gt, max_fpr=0.30))
        except Exception as e:
            logger.warning(f"无法计算子集 AU-PRO（样本数={len(indices)}）: {e}")

        # F1 计算（多个阈值，仅当需要时）
        if compute_f1:
            for threshold in args.f1_thresholds:
                try:
                    f1_result = pixel_f1(sub_pr, sub_gt, threshold=threshold)
                    f1_results_dict[threshold] = f1_result
                except Exception as e:
                    logger.warning(f"无法计算子集 F1（样本数={len(indices)}, 阈值={threshold}）: {e}")
                    f1_results_dict[threshold] = None

        return len(indices), seg_auc, sub_au_pro_005, sub_au_pro_030, f1_results_dict

    # 统计各子集
    # 为 A、B 及其困难项添加多个阈值下的 F1、Precision、Recall 列
    stats_rows = {
        "Category": [],
        "Count": [],
        "Seg-AUC": [],
    ...
    # 为每个阈值添加 F1、Precision、Recall 列
    f1_thresholds = args.f1_thresholds
    for threshold in f1_thresholds:
        stats_rows[f"F1@{threshold:.3f}"] = []
        stats_rows[f"Precision@{threshold:.3f}"] = []
        stats_rows[f"Recall@{threshold:.3f}"] = []

    display_order = ["A", "B", "C", "A_hard", "B_hard", "C_hard"]
    name_map = {
        "A": "A",
        "B": "B",
        "C": "C",
    ...
    for key in display_order:
        indices = subsets.get(key, [])
        # 仅对 A、B 及其困难项计算 F1
        compute_f1 = key in ["A", "B", "A_hard", "B_hard"]
        count, seg_auc_val, au_pro_005_val, au_pro_030_val, f1_results_dict = compute_subset_metrics(indices, compute_f1=compute_f1)
        if count == 0:
            continue

        stats_rows["Category"].append(name_map.get(key, key))
        stats_rows["Count"].append(count)
        stats_rows["Seg-AUC"].append(seg_auc_val * 100.0)
        stats_rows["AU-PRO@0.05"].append(au_pro_005_val * 100.0)
        stats_rows["AU-PRO@0.30"].append(au_pro_030_val * 100.0)
        
        # 为每个阈值添加 F1、Precision、Recall 值
        for threshold in f1_thresholds:
            f1_col = f"F1@{threshold:.3f}"
            prec_col = f"Precision@{threshold:.3f}"
            recall_col = f"Recall@{threshold:.3f}"
            
            if compute_f1 and threshold in f1_results_dict and f1_results_dict[threshold] is not None:
                f1_result = f1_results_dict[threshold]
                stats_rows[f1_col].append(f1_result.f1 * 100.0)
                stats_rows[prec_col].append(f1_result.precision * 100.0)
                stats_rows[recall_col].append(f1_result.recall * 100.0)
            else:
                # 对于 C、C_hard 或不计算 F1 的情况，填充 "-"
                stats_rows[f1_col].append("-")
                stats_rows[prec_col].append("-")
                stats_rows[recall_col].append("-")

    if len(stats_rows["Category"]) > 0:
        # 格式化所有数值列
        stats_rows_formatted = {}
        for col_name, col_values in stats_rows.items():
            if col_name == "Category":
                stats_rows_formatted[col_name] = col_values
            else:
                formatted_vals = []
                ...
                stats_rows_formatted[col_name] = formatted_vals
        ...
        logger.info("\nA/B/C 类及其困难项统计表（包含多个阈值下的 F1、Precision、Recall）：")
        logger.info("\n" + table_abc)
    else:
        logger.info("A/B/C 类及其困难项中无符合条件的样本。")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--f1_thresholds", type=float, nargs="+", default=[0.2], help="Thresholds for pixel-level F1 calculation (can specify multiple)")
    parser.add_argument("--enable_controlnet", action="store_true", help="Enable ControlNet at inference")
    parser.add_argument("--control_hint_source", type=str, default="image", choices=["gt", "image"], help="Control hint source at inference")
